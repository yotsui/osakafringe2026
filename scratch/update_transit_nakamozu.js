...
async function main() {
  const zipPath = 'scratch/N02-23_GML.zip';
  const extractPath = 'scratch/n02';
  ...
  if (!fs.existsSync(zipPath)) {
    ...
    console.log('Downloaded N02-23_GML.zip.');
  }

  // 2. Unzip if not exists
  if (!fs.existsSync('scratch/n02/UTF-8/N02-23_RailroadSection.geojson')) {
  ...
  // Extended bounding box to cover Nakamozu (中百舌鳥 34.556), Sakai-higashi, Sakai, and southern Osaka Metro terminals
  const BBOX = {
    minLng: 135.30,
    maxLng: 135.65,
    minLat: 34.50, // extended to south for Nakamozu & Sakai
    maxLat: 34.82,
  };
  ...
  console.log('Extracted lines features:', outLines.length);
  ...
  console.log('Extracted unique stations:', outStations.length);
  const nakamozuStations = outStations.filter(s => s.properties.name.includes('なかもず') || s.properties.name.includes('中百舌鳥'));
  console.log('Nakamozu stations found:', nakamozuStations.map(s => `${s.properties.name} (${s.properties.symbol}, ${s.geometry.coordinates})`));
 ...
 * - Osaka Metro 9路線（御堂筋線なかもず駅まで完全延伸）
 * - 南海電鉄（南海本線・高野線・中百舌鳥駅方面まで延伸）
 * - JR西日本（#333333 濃いグレー）
...
`;
  ...
  console.log('Updated src/data/transitData.ts successfully. Size:', tsContent.length);

  // Clean up
  console.log('Cleaning up scratch temp files...');
